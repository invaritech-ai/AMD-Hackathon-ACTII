...
export function SupplierBreakdown({ runs }: SupplierBreakdownProps) {
  const data = useMemo(() => {
    ...
    runs
      ...
      .forEach((r) => {
        supplierMap.set(r.supplier_name, (supplierMap.get(r.supplier_name) || 0) + r.total_claim_value);
      });
        ...
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={data} layout="vertical" margin={{ left: 20, right: 30, top: 5, bottom: 5 }}>
            ...
            <Tooltip
              ...
              formatter={(value: number) => [`$${value.toFixed(2)}`, "Claim Value"]}
              cursor={{ fill: "rgb(139 92 246 / 0.06)" }}
            />
